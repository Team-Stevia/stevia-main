...
export class BoardsService {
   ...
   */
    // 강의실 예약 비즈니스 로직
    async reserveRoom(reserveRoomDto: ReserveRoomDto, studentId: number): Promise<{reserveId: string}> {
        const {
            usageTime, roomId,
        } = reserveRoomDto;

        // 학생 학번통해서 예약자 id 갖고 오기
        const reserverId = await this.boardsRepository.getReserverIdByStudentId(studentId);
